...
def _score(window: LocationWindow, workload: Workload, max_price: Decimal, max_carbon: Decimal) -> Decimal:
...
def defer(window_list: list[LocationWindow], workload: Workload, current: LocationWindow) -> LocationWindow:
    if not workload.batch or workload.deadline_window <= 0:
        return current
    selected = choose(window_list, workload)
    return selected if _score(selected, workload, max(w.price_per_unit for w in window_list),
                               max(w.carbon_intensity for w in window_list)) < _score(current, workload, max(w.price_per_unit for w in window_list),
                                                                                     max(w.carbon_intensity for w in window_list)) else current
