...
def recommendations(allocations: Iterable[Allocation]) -> list[str]:
    by_tenant: dict[str, Decimal] = {}
    for item in allocations:
        by_tenant[item.tenant] = by_tenant.get(item.tenant, Decimal(0)) + item.driver
    return [
        f"rightsizing candidate: {tenant} has {driver} shared-driver units"
        for tenant, driver in by_tenant.items()
        if driver == 0
    ]
