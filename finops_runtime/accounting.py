...
def allocate(usage: Iterable[Usage], pools: Iterable[SharedPool]) -> list[Allocation]:
    ...
    for item in usages:
        ...
        allocations.append(Allocation(item.tenant, item.service, item.resource,
                                      item.direct_cost, shared, carbon, item.driver, source, item.requests))
    return allocations
...
def reconcile(allocations: Iterable[Allocation], pools: Iterable[SharedPool]) -> dict[str, Decimal]:
    ...
    expected = {p.resource: p.cost for p in pools}
    return {resource: expected.get(resource, Decimal("0")) - amount
            for resource, amount in allocated.items()}


def pool_report(allocations: Iterable[Allocation], pools: Iterable[SharedPool]) -> list[dict[str, Decimal | str]]:
    ...
    for pool in pools:
        ...
        rows.append({"pool": pool.pool_id, "resource": pool.resource,
                     "allocated_driver": allocated, "idle_driver": pool.idle_capacity,
                     "unallocated_driver": max(pool.capacity - allocated - pool.idle_capacity, Decimal("0"))})
    return rows


def tenant_totals(allocations: Iterable[Allocation]) -> dict[str, dict[str, Decimal]]:
    totals: dict[str, dict[str, Decimal]] = defaultdict(lambda: {"cost": Decimal("0"), "carbon": Decimal("0"), "requests": Decimal("0")})
    for item in allocations:
