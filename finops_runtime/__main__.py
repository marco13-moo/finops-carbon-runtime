...
def demo() -> None:
    ...
    pools = [SharedPool("node-pool-a", "kubernetes", Decimal("10"), Decimal("1000"), Decimal("100")),
             SharedPool("db-a", "database", Decimal("5"), Decimal("400"), Decimal("10"))]
    allocations = allocate(usage, pools)
    for item in allocations:
        print(f"{item.tenant:6} {item.service:8} {item.resource:15} cost=${item.total_cost:.4f} carbon={item.carbon_grams:.4f} source={item.source}")
